...
async def test_client_response_handling():
    ...
    futures[0].set_result(server_error_response)
    futures[1].set_result(server_error_response)
    futures[2].set_result(success_response)
    mock_func.side_effect = futures
    ...
    mock_func = Mock()
    future = asyncio.Future()
    future.set_result(client_error_response)
    mock_func.return_value = future
    decorated = create_retry_decorator()(mock_func)
